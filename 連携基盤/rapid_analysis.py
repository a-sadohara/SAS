...
#                    socket情報
# ------------------------------------------------------------------------------------
def get_rapid_server_connect_info_worker(ip_address, port_numbers, retry_num, logger):
    result = False
    sock = []
...
#
# ------------------------------------------------------------------------------------
def get_rapid_server_connect_info_host(ip_address, port_numbers, worker, retry_num, logger):
    result = False
    sock = []
    ...
    try:
        ...
        for i in range(worker):
            ...
            tmp_result, sock_work = get_rapid_server_connect_info_worker(ip_address, port_number, retry_num, logger)
...
#
# ------------------------------------------------------------------------------------
def get_rapid_server_connect_info(ip_addresses, port_numbers, worker, retry_num, logger):
    result = False
    sock_all = []
    ...
    try:
        ...
        for i in range(len(ip_addresses)):
            ...
            tmp_result, sock = \
                get_rapid_server_connect_info_host(ip_address, port_numbers, worker, retry_num, logger)
...
#                    カーソルオブジェクト
# ------------------------------------------------------------------------------------
def select_fabric_info_db_polling(conn, cur, processing_status_status, unit_num):
    ### クエリを作成する
    sql = 'select' \
          ...
          '  order by processing_status.resize_endtime asc, fabric_info.imaging_starttime asc' \
          % (processing_status_status, unit_num)
...
#                    カーソルオブジェクト
# ------------------------------------------------------------------------------------
def update_fabric_info(column_name, time, status, fabric_name, inspection_num, cur, conn, imaging_starttime, unit_num):
    ### クエリを作成する
    sql = 'update fabric_info set status = %s, %s = \'%s\' ' \
          'where fabric_name = \'%s\' and inspection_num = %s and imaging_starttime = \'%s\' and unit_num = \'%s\'' \
          % (status, column_name, time, fabric_name, inspection_num, imaging_starttime, unit_num)

    logger.debug('[%s:%s] 反物情報テーブルステータス更新SQL %s' % (app_id, app_name, sql))
    ### 反物情報テーブルを更新
...
#                    カーソルオブジェクト
# ------------------------------------------------------------------------------------
def select_processing_status_rapid_target(conn, cur, fabric_name, inspection_num, rapid_host_name,
                                          status_resize_end, imaging_starttime, unit_num, logger):
    ### クエリを作成する
    sql = 'select processing_id from processing_status ' \
          'where fabric_name = \'%s\' and inspection_num = %s and rapid_host_name = \'%s\' ' \
          'and status = %s and imaging_starttime = \'%s\' and unit_num = \'%s\'' \
          % (fabric_name, inspection_num, rapid_host_name, status_resize_end, imaging_starttime, unit_num)
...
def update_processing_status(time, status, fabric_name, inspection_num, processing_id, rapid_server_host_name,
                             cur, conn, imaging_starttime, unit_num, logger):
    ### クエリを作成する
    sql = 'update processing_status set status = %s, rapid_starttime = \'%s\'' \
          '  where fabric_name = \'%s\' and inspection_num = %s and processing_id = %s ' \
          'and rapid_host_name = \'%s\' and imaging_starttime = \'%s\' and unit_num = \'%s\'' \
...
#                    ソケット情報
# ------------------------------------------------------------------------------------
def process(sock, request_json, ip_address, port_num, retry_num, fabric_name, inspection_num, today_date,
            processing_id):
    result = False
    received_str = None
    ### ログ設定
    logger_name = "rapid_analysis_" + str(ip_address) + "_" + str(port_num)
    logger_subproc = logging.getLogger(logger_name)
    ...
    try:
        ...
        for x in range(retry_num):
            ...
            logger_subproc.info('[%s:%s] process: exec time:[%s] [反番, 検査番号, 検査日付, 処理ID]=[%s, %s, %s, %s]', app_id,
                                app_name, end - start, fabric_name, inspection_num, today_date, processing_id)

            # エラーコード判定
            result_data = json.loads(received_str)
            response_check = 'result_list' in result_data
            if response_check:
            ...
            if result_data_len > 0:
                for j in range(result_data_len):
                    ...
                    logger_subproc.debug('[%s:%s] 処理結果（IMA_status）:[%s]', app_id, app_name, ima_status)
                    ...
                    if ima_status == '0':
                        ...
                        if x < (retry_num - 1):
                            ...
                            if ima_status in retry_target_err_no:
                                ...
                                logger_subproc.warning('[%s:%s] RAPID解析処理をリトライします。:[%s]', app_id, app_name, ima_status)
                                # socket再接続
                                worker_result, sock = \
                                ...
                                # 上記以外のエラー
                                logger_subproc.error('[%s:%s] RAPID解析処理が異常終了しました。:[%s]', app_id, app_name, ima_status)
                                return result, received_str, sock
                        else:
                            # リトライ回数超過の場合
                            logger_subproc.error('[%s:%s] RAPID解析処理が異常終了しました。（リトライ回数超過）:[%s]', app_id, app_name,
                                                 ima_status)
                            return result, received_str, sock
        ...
        # 失敗時は共通例外関数でエラー詳細をログ出力する
        error_detail.exception(e, logger_subproc, app_id, app_name)
...
def ng_result_register(received_str, product_name, fabric_name, inspection_num, processing_id, start_time,
                       end_time, rapid_server_host_name, image_dir, today_date, response_json_file_path,
                       i, conn, cur, imaging_starttime, unit_num, logger):
    result = False
    ...
    if response_check:
        ...
        result_data_len = len(result_data['result'])
    ...
    if result_data_len > 0:
        for i in range(result_data_len):
            ...
            if response_check:
                ...
                result_list = [result_data['result'][i]]
                open_file = codecs.open(
                    response_json_file_path + '\\' + today_date + '_' + product_name + '_' + fabric_name + '_' + str(
                        inspection_num).zfill(2) + '_' + str(processing_id) + '_' + str(worker + 1) + '_request.txt',
                    'r', 'utf-8')
                json_data = json.load(open_file)
                image_file = json_data["image"][0]
                open_file.close()
                results = result_list

            # logger.debug('[%s:%s] [%s]:[result_list=[%s]]', app_id, app_name, i, result_list)
            logger.debug('[%s:%s] [%s]:[image_file=[%s],[results=[%s]]', app_id, app_name,
                         i, image_file, results)
            ...
            if len(results) > 0:
                for j in range(len(results)):
                    ...
                    if result_status == 'NG' and confidence >= analysis_ng_Threshold:
                        ...
                        if face_no == '1':
                            # 検版部No1の場合
                            camera_no_1 = file_name[5]
                            camera_no_2 = None
                        else:
                            # 検版部No2の場合
                            camera_no_1 = None
                            camera_no_2 = file_name[5]
    ...
    elif result_data_len == 0 and result_data['IMA_status'] == '0':
        logger.debug('[%s:%s] RAPID解析結果(1枚)は正常です。', app_id, app_name)
        # ここまで到達する場合は正常終了
        result = True
...
#                    カーソルオブジェクト
# ------------------------------------------------------------------------------------
def select_inspection_info(conn, cur, fabric_name, inspection_num, timestamp, imaging_starttime, unit_num, logger):
    ### クエリを作成する
    sql = 'select worker_1, worker_2 from inspection_info_header ' \
          'where fabric_name = \'%s\' and inspection_num = %s and start_datetime = \'%s\' and unit_num = \'%s\' ' \
          'and insert_datetime <= \'%s\' order by insert_datetime desc' \
          % (fabric_name, inspection_num, imaging_starttime, unit_num, timestamp)
...
def insert_rapid_analysis_info(product_name, fabric_name, camera_num_1, camera_num_2, inspection_num, processing_id,
                               image_file, start_time, end_time, result_status, result_position, confidence,
                               face_no, rapid_server_host_name, image_name, cur, conn, imaging_starttime, unit_num,
                               logger):
    ...
    tmp_result, records, conn, cur = select_inspection_info(conn, cur, fabric_name, inspection_num,
                                                            file_timestamp, imaging_starttime, unit_num, logger)
    if tmp_result:
    ...
    sql = 'insert into "rapid_%s_%s_%s" (' \
          ...
          % (fabric_name, inspection_num, inspection_date, product_name, fabric_name, camera_num_1, camera_num_2,
             inspection_num, processing_id, image_file, start_time, end_time, result_status, result_position,
             confidence, rapid_server_host_name, worker_1, worker_2, '#' + face_no, unit_num)

    logger.debug('[%s:%s] RAPID解析情報テーブル登録SQL %s' % (app_id, app_name, sql))
...
def update_processing_status_rapid_end(rapid_ng_num, rapid_processed_num,
    ...
    sql = 'update processing_status set status = %s, rapid_ng_num = %s, rapid_processed_num = %s, ' \
          'rapid_endtime = \'%s\' where fabric_name = \'%s\' and inspection_num = %s and processing_id = %s ' \
          'and rapid_host_name = \'%s\' and imaging_starttime = \'%s\' and unit_num = \'%s\'' \
...
def exec_rapid_analysis_multi_thread(rapid_server_host_name, worker, port_num,
                                     ...
                                     imaging_starttime,
                                     sock, ai_model_name,
                                     all_processed_num, all_ng_num, unit_num):
    ### ログ設定
    logger_name = "rapid_analysis_" + str(rapid_server_host_name)
    logger_subthread = logging.getLogger(logger_name)

    logger_subthread.debug('[%s:%s] exec_rapid_analysis_multi_thread start'
    ...
    try:
        ...
        if tmp_result:
            ...
            return result, all_processed_num, all_ng_num

        ### 処理ID取得
        # DB共通処理を呼び出して、処理ステータステーブルから検査情報を取得する。
        logger_subthread.debug('[%s:%s] 処理ステータスデータ取得（RAPID解析対象）を開始します。', app_id, app_name)
        tmp_result, records, conn, cur = \
            select_processing_status_rapid_target(conn, cur, fabric_name, inspection_num,
                                                  rapid_server_host_name, status_resize_end, imaging_starttime,
                                                  unit_num, logger_subthread)

        if tmp_result:
            logger_subthread.debug('[%s:%s] 処理ステータスデータ取得（RAPID解析対象）が終了しました。', app_id, app_name)
            conn.commit()
        else:
            logger_subthread.error('[%s:%s] 処理ステータスデータ取得（RAPID解析対象）に失敗しました。', app_id, app_name)
            return result, all_processed_num, all_ng_num

        if len(records) > 0:
            logger_subthread.debug('[%s:%s] RAPID解析対象が存在します。', app_id, app_name)
        else:
            logger_subthread.debug('[%s:%s] RAPID解析対象が存在しません。', app_id, app_name)
            result = True
            return result, all_processed_num, all_ng_num
        ...
        for i in range(len(records)):
            ...
            ### 処理ステータス更新
            logger_subthread.debug('[%s:%s] 処理ステータステーブル更新を開始します。', app_id, app_name)
            tmp_result, conn, cur = update_processing_status(start_time, processing_status_rapid_start, fabric_name,
                                                             inspection_num, processing_id, rapid_server_host_name, cur,
                                                             conn, imaging_starttime, unit_num, logger_subthread)

            if tmp_result:
                logger_subthread.debug('[%s:%s] 処理ステータステーブルの更新が終了しました。', app_id, app_name)
                conn.commit()
            else:
                logger_subthread.error('[%s:%s] 処理ステータステーブルの更新に失敗しました。', app_id, app_name)
                conn.rollback()
                return result, all_processed_num, all_ng_num
            ...
            image_file_name_pattern = '\\*' + fabric_name + '_' + today_date + '_' + str(inspection_num).zfill(
                (2)) + image_file_pattern
            ...
            logger_subthread.debug('[%s:%s] 検知対象画像ファイルの確認を開始します。', app_id, app_name)
            tmp_result, image_files = get_file(image_path, image_file_name_pattern, logger_subthread)

            if tmp_result:
                logger_subthread.debug('[%s:%s] 検知対象画像ファイルの確認が完了しました。', app_id, app_name)
            else:
                logger_subthread.error('[%s:%s] 検知対象画像ファイルの確認に失敗しました。', app_id, app_name)
                conn.rollback()
                return result, all_processed_num, all_ng_num

            logger_subthread.debug('[%s:%s] 検知対象画像ファイル:[%s]', app_id, app_name, image_files)
            ...
            for i in range(0, worker):
                ...
                request_json_file_name = today_date + '_' + product_name + '_' + fabric_name + '_' + str(
                    inspection_num).zfill((2)) + '_' + str(processing_id) + '_' + str(i + 1) + '_request.txt'
                logger_subthread.debug('[%s:%s] request_json_file_name:[%s]', app_id, app_name, request_json_file_name)
                ...
                request_json_file_path = rk_root_path + '\\JSON\\' + today_date + '_' + product_name + '_' + \
                                         ...
                                         "\\" + rapid_server_host_name \
                                         + "\\" + str(processing_id)
                logger_subthread.debug('[%s:%s] request_json_file_path:[%s]', app_id, app_name, request_json_file_path)

                # 格納先フォルダを作成する
                logger_subthread.debug('[%s:%s] JSONファイル格納フォルダ作成を開始します。', app_id, app_name)
                tmp_result = exists_dir(request_json_file_path, logger_subthread)

                if tmp_result:
                    logger_subthread.debug('[%s:%s] JSONファイル格納フォルダ作成が完了しました。', app_id, app_name)
                else:
                    logger_subthread.error('[%s:%s] JSONファイル格納フォルダ作成に失敗しました。', app_id, app_name)
                    conn.rollback()
                    return result, all_processed_num, all_ng_num

                # JSONファイル作成
                logger_subthread.debug('[%s:%s] リクエストJSONファイル作成を開始します。:[%s]', app_id, app_name,
                ...
                ofp.close()
                logger_subthread.debug('[%s:%s] リクエストJSONファイル作成が完了しました。:[%s]', app_id, app_name,
                                       request_json_file_path + '\\' + request_json_file_name)
            ...
            # 引数の準備
            process_args = [
                [sock[i], request_jsons[i], rapid_server_host_name, port_num[i], int(rapid_connect_num), fabric_name,
                 inspection_num, today_date, processing_id] for i in range(0, worker)]

            ### RAPID解析実行
            logger_subthread.info('[%s:%s] RAPID解析実行を開始します。 [反番, 検査番号, 検査日付, 処理ID]=[%s, %s, %s, %s]',
                                  app_id, app_name, fabric_name, inspection_num, today_date, processing_id)
            ...
            for i in range(0, worker):
                ...
                logger_subthread.info('[%s:%s] received_data:[%s] [反番, 検査番号, 検査日付, 処理ID]=[%s, %s, %s, %s]',
                                      app_id, app_name, tmp_result, fabric_name, inspection_num, today_date,
                                      processing_id)
                if tmp_result:
                    # 正常終了時
                    if received_data[i][1] is None or received_data[i][1] == '':
                        logger_subthread.error('[%s:%s] RAPID解析結果の取得に失敗しました。', app_id, app_name)
                        conn.rollback()
                        return result, all_processed_num, all_ng_num
                    else:
                        # 子スレッドからの結果を変数に設定
                    ...
                    # 異常終了時
                    logger_subthread.error('[%s:%s] RAPID解析実行に失敗しました。', app_id, app_name)
                    conn.rollback()
                    return result, all_processed_num, all_ng_num
            ...
            for i in range(0, worker):
                ...
                response_json_file_name = today_date + '_' + product_name + '_' + fabric_name + '_' + str(
                    inspection_num).zfill((2)) + '_' + \
                                          str(processing_id) + '_' + str(i + 1) + '_response.txt'
                logger_subthread.debug('[%s:%s] response_json_file_name:[%s]', app_id, app_name,
                                       response_json_file_name)

                # 参照先パス構成は以下を想定する。
                # リクエストJSONファイル格納と同じパス
                request_json_file_path = rk_root_path + '\\JSON\\' + today_date + '_' + product_name + '_' + \
                                         fabric_name + '_' + str(inspection_num).zfill(
                    (2)) + "\\" + rapid_server_host_name \
                                         + "\\" + str(processing_id)

                response_json_file_path = request_json_file_path
                logger_subthread.debug('[%s:%s] response_json_file_path:[%s]', app_id, app_name,
                                       response_json_file_path)

                # JSONファイル作成
                logger_subthread.debug('[%s:%s] レスポンスJSONファイルの作成を開始します。:[%s]', app_id, app_name,
                ...
                ofp.close()
                logger_subthread.debug('[%s:%s] レスポンスJSONファイルの作成が完了しました。:[%s]', app_id, app_name,
                                       response_json_file_path + '\\' + response_json_file_name)
                ...
                tmp_result, conn, cur, rapid_processed_num, rapid_ng_num, ng_file_relative_path_list \
                    = ng_result_register(received_str[i], product_name, fabric_name, inspection_num, processing_id,
                                         start_time, end_time, rapid_server_host_name, image_dir,
                                         today_date, response_json_file_path, i, conn, cur, imaging_starttime, unit_num,
                                         logger_subthread)
                all_ng_num += rapid_ng_num
                all_processed_num += rapid_processed_num

                if tmp_result:
                    logger_subthread.debug('[%s:%s] NG結果登録が完了しました。', app_id, app_name)
                else:
                    logger_subthread.error('[%s:%s] NG結果登録に失敗しました。', app_id, app_name)
                    conn.rollback()
                    return result, all_processed_num, all_ng_num
                ...
                if len(ng_file_relative_path_list) > 0:
                    ...
                    # コピー先フォルダを作成する
                    logger_subthread.debug('[%s:%s] NG画像ファイルのコピー先フォルダ作成を開始します。:コピー先フォルダ[%s]', app_id, app_name,
                                           copy_path)
                    tmp_result = exists_dir(copy_path, logger_subthread)

                    if tmp_result:
                        logger_subthread.debug('[%s:%s] NG画像ファイルのコピー先フォルダ作成が完了しました。:コピー先フォルダ[%s]', app_id, app_name,
                                               copy_path)
                    else:
                        logger_subthread.error('[%s:%s] NG画像ファイルのコピー先フォルダ作成に失敗しました。:コピー先フォルダ[%s]', app_id, app_name,
                                               copy_path)
                        conn.rollback()
                        return result, all_processed_num, all_ng_num

                    for i in range(len(ng_file_relative_path_list)):
                        logger_subthread.debug('[%s:%s] NG画像コピーを開始します。 ', app_id, app_name)
                        ...
                        tmp_result = copy_file(ng_file_path, copy_path, logger_subthread)

                        if tmp_result:
                            logger_subthread.debug('[%s:%s] NG画像コピーが完了しました。 コピー先フォルダ[%s], NG画像ファイル名[%s]',
                                                   app_id, app_name, copy_path, ng_file_path)
                        else:
                            logger_subthread.error('[%s:%s] NG画像コピーに失敗しました。 コピー先フォルダ[%s], NG画像ファイル名[%s]',
                                                   app_id, app_name, copy_path, ng_file_path)
                            conn.rollback()
                            return result, all_processed_num, all_ng_num

            ### 処理ステータス更新
            logger_subthread.debug('[%s:%s] 処理ステータス更新を開始します。', app_id, app_name)
            end_time = datetime.datetime.now()
            tmp_result, conn, cur = \
                update_processing_status_rapid_end(all_ng_num, all_processed_num, fabric_name, inspection_num,
                                                   ...
                                                   imaging_starttime, unit_num, logger_subthread)

            if tmp_result:
                logger_subthread.debug('[%s:%s] 処理ステータス更新が終了しました。', app_id, app_name)
            else:
                logger_subthread.error('[%s:%s] 処理ステータス更新に失敗しました。', app_id, app_name)
                conn.rollback()
                return result, all_processed_num, all_ng_num

            conn.commit()

            logger_subthread.debug('[%s:%s] 処理済枚数確認を開始します。', app_id, app_name)
            if all_processed_num >= image_num_threshold:
                logger_subthread.debug('[%s:%s] 処理済枚数確認が終了しました。画像枚数閾値を超えています。', app_id, app_name)
                logger_subthread.debug('[%s:%s] 大量検知判定を開始します。', app_id, app_name)
                # 処理結果を確認する
                ...
                if all_processed_num >= image_num_threshold and ng_rate >= ng_threshold:
                    ...
                    # 大量検知として異常終了
                    logger_subthread.warning('[%s:%s] 大量検知です。:処理済枚数[%s] NG件数[%s] NG率[%s] ', app_id, app_name,
                                             all_processed_num, all_ng_num, ng_rate)
                    result = True
                    #return result, all_processed_num, all_ng_num

                else:
                    logger_subthread.debug('[%s:%s] 大量検知ではありません。', app_id, app_name)
                    logger_subthread.info('[%s:%s] RAPID解析のマルチスレッド実行を終了します。 [反番, 検査番号, 検査日付, 処理ID]=[%s, %s, %s, %s]',
                                          app_id, app_name, fabric_name, inspection_num, today_date,
                                          processing_id)
                    result = True
        ...
        # 失敗時は共通例外関数でエラー詳細をログ出力する
        error_detail.exception(e, logger_subthread, app_id, app_name)

    logger_subthread.debug('[%s:%s] exec_rapid_analysis_multi_thread end'
                           '[rapid_server_host_name:%s][worker:%s][port_num:%s]'
                           '[product_name:%s][fabric_name:%s][inspection_num:%s][imaging_starttime:%s]',
                           app_id, app_name,
                           rapid_server_host_name, worker, port_num,
...
#                    カーソルオブジェクト
# ------------------------------------------------------------------------------------
def select_fabric_info_separateresize_endtime(conn, cur, fabric_name, inspection_num, imaging_starttime, unit_num):
    ### クエリを作成する
    sql = 'select separateresize_endtime from fabric_info' \
          ' where fabric_name = \'%s\' and inspection_num = %s and imaging_starttime = \'%s\' and unit_num = \'%s\'' \
          % (fabric_name, inspection_num, imaging_starttime, unit_num)

    logger.debug('[%s:%s] 反物情報取得（分割/リサイズ完了時刻）SQL %s' % (app_id, app_name, sql))
    # DB共通処理を呼び出して、処理ステータステーブルと反物情報テーブルからデータを取得する。
...
#                    カーソルオブジェクト
# ------------------------------------------------------------------------------------
def select_processing_status_rapid_end(conn, cur, fabric_name, inspection_num, imaging_starttime, unit_num):
    ### クエリを作成する
    sql = 'select ' \
          'count(*) as record_count ,' \
          'count(case when rapid_endtime is not null then 1 else null end) as rapid_end_count' \
          ' from processing_status where fabric_name = \'%s\' and inspection_num = \'%s\' ' \
          'and imaging_starttime = \'%s\' and unit_num = \'%s\' group by fabric_name, inspection_num' \
          % (fabric_name, inspection_num, imaging_starttime, unit_num)

    logger.debug('[%s:%s] 処理ステータス情報取得（RAPID解析処理完了）SQL %s' % (app_id, app_name, sql))
    # DB共通処理を呼び出して、処理ステータステーブルからデータを取得する。
...
# 戻り値             ：処理結果（True:成功、False:失敗）
# ------------------------------------------------------------------------------------
def start_rapid_process(bat_file_path, start_bat_file_name, logger):
    result = False
    try:
        # コマンド作成
        cmd = bat_file_path + "/" + start_bat_file_name
        # 常駐プロセス起動バッチ実行
...
# 戻り値             ：処理結果（True:成功、False:失敗）
# ------------------------------------------------------------------------------------
def stop_rapid_process(bat_file_path, stop_bat_file_name, logger):
    result = False
    try:
        # コマンド作成
        cmd = bat_file_path + "/" + stop_bat_file_name
        # 常駐プロセス起動バッチ実行
        tmp_result = subprocess.Popen(['start', cmd], shell=True)
        test = tmp_result.communicate()[0]
        return_code = tmp_result.returncode
...
def main():
    ...
    try:
        ...
        # 初回処理時変数
        first_execute_flag = 0

        # RAPID常駐プロセス起動バッチファイルパス
        bat_file_path = inifile.get('FILE_PATH', 'bat_file_path')
        # RAPID常駐プロセス起動バッチ名
        start_bat_file_name = inifile.get('FILE_NAME', 'start_bat_file_name')
        # RAPID常駐プロセス起動バッチ名
        stop_bat_file_name = inifile.get('FILE_NAME', 'stop_bat_file_name')
        ...
        while True:
            ...
            while True:
                ...
                result, target_record, conn, cur = \
                    select_fabric_info_db_polling(conn, cur, processing_status_resize_end, unit_num)
                ...
                if result:
                    ...
                    conn.rollback()
                    sys.exit()
                logger.debug(target_record)
                #logger.debug(len(target_record))
                # 取得結果から、該当レコードが存在するか確認する。
                if target_record is not None and len(target_record) > 0:
                    # 該当レコードが存在する場合
                    logger.debug('該当レコードが存在する場合')
                    ...
                    ### 常駐プロセス起動
                    result = start_rapid_process(bat_file_path, start_bat_file_name, logger)
                    if result:
                        logger.debug('[%s:%s] RAPID常駐プロセス起動が終了しました。', app_id, app_name)
                    else:
                        logger.error('[%s:%s] RAPID常駐プロセス起動でエラーが発生しました。', app_id, app_name)
                        sys.exit()

                    ### 常駐プロセス接続
                    ip_address = ip_addresses.split(',')
                    result, sock = get_rapid_server_connect_info(ip_address, port_numbers, worker, rapid_connect_retry_num, logger)
                    if result:
                    ...
                    logger.debug('[%s:%s] マルチプロセスを開始します。', app_id, app_name)               # マルチスレッド実行
                        ...
                        for i in range(thread_num):
                            ...
                            func_list.append(
                                executor.submit(
                                    ...
                                    multi_param_product_name[i], multi_param_fabric_name[i],
                                    multi_param_inspection_num[i],
                                    multi_param_rapid_starttime[i],
                                    sock[i], ai_model_name[0],
                                    multi_processed_num[i], multi_ng_num[i], unit_num))
                            time.sleep(1)
                        for i in range(thread_num):
                            # スレッド戻り値を取得
                            result_list.append(func_list[i].result())

                    logger.info('[%s:%s] マルチプロセスが終了しました。 [反番, 検査番号, 検査日付]=[%s, %s, %s]',
                    ...
                    for i, multi_result in enumerate(result_list):
                        if multi_result[0] is True:
                            logger.debug('[%s:%s] multi_result【マルチスレッド[%d]】:正常終了しました。[%s]', app_id, app_name, i,
                                         str(multi_result))
                            multi_processed_num[i] = multi_result[1]
                            multi_ng_num[i] = multi_result[2]

                        else:
                            # 異常終了しているものが存在する場合
                            logger.error('[%s:%s] multi_result【マルチスレッド[%d]】:異常終了しました。[%s]', app_id, app_name, i,
                                         str(multi_result))
                            conn.rollback()
                            sys.exit()
                    ...
                    ### 常駐プロセス停止
                    result = stop_rapid_process(bat_file_path, stop_bat_file_name, logger)
                    if result:
                        logger.debug('[%s:%s] RAPID常駐プロセス起動が終了しました。', app_id, app_name)
                    else:
                        logger.error('[%s:%s] RAPID常駐プロセス起動でエラーが発生しました。', app_id, app_name)
                    ...
                    # 該当レコードが無い場合
                    logger.debug('該当レコードが無い場合')
                    logger.info('[%s:%s] 処理対象の反物情報が存在しません。',
                                app_id, app_name)
                    if first_execute_flag == 0:
                ...
                if records_fabric[0] is None:
                    logger.info('[%s:%s] 処理単位分割、画像リサイズが終了していません。 [反番, 検査番号, 検査日付]=[%s, %s, %s]',
                                app_id, app_name, fabric_name, inspection_num, inspection_date)
                    ...
                    if record_processing[0] == record_processing[1]:
                        ...
                        if confirm_error_flag == error_continue_num:
                            ...
                            logger.info('[%s:%s] RAPID解析処理対象が残っています。 [反番, 検査番号, 検査日付]=[%s, %s, %s]',
                                         app_id, app_name, app_name, fabric_name, inspection_num, inspection_date)
