...
#                      処理済み枚数
# ------------------------------------------------------------------------------------
def separate_image(file_list, output_path, file_num, rapid_host_name, logger, fabric_name, inspection_num, date):
    result = False
    try:
...
def confirm_image_num(file_list, max_file, endtime, output_path, rapid_host_name, logger,
                      fabric_name, inspection_num, date):
...
def exec_separate_process_unit_multi_thread(product_name, fabric_name, inspection_num, image_dir,
    ...
    try:
        ...
        if tmp_result:
            ...
            return result, processed_num, processed_id, rapid_host_name, file_list
        ...
        tmp_result, file_list = monitor_image_file(image_dir, image_file_name, logger_subprocess)
        ...
        if len(file_list) > 0:
            ...
            for x in range(file_length):
                ...
                tmp_result, file_list, file_num = confirm_image_num(file_list, max_file, imaging_endtime,
                                                                    ...
                                                                    logger_subprocess, fabric_name, inspection_num, date)
                    ...
                    if ai_model_flag == 1:
                        ...
                        if tmp_result:
                            ...
                            logger_subprocess.debug('[%s:%s] RAPID学習用画像の移動を開始します。 ホスト名=[%s]'
                                                    % (app_id, app_name, rapid_host_name))
                            tmp_result = file_util.make_directory(ai_model_path, logger_subprocess, app_id, app_name)
                            if tmp_result:
                                pass
                            else:
                            ...
                            for i in range(len(move_file_list)):
                                tmp_result = file_util.move_file(move_file_list[i], ai_model_path, logger_subprocess,
                                                                 app_id, app_name)
...
def confirm_processed(conn, cur, fabric_name, inspection_num, image_num, processed_num, imaging_endtime, column, time,
                      ai_flag, confirm_errorflg, status, error_continue_num, imaging_starttime, unit_num, file_list_num):
                ...
                if result is True:
                    ...
                    if ai_flag == 1:
                        ...
                        if result:
                            ...
                            logger.error('[%s:%s] パトライト点灯処理が失敗しました。 '
                                         '[反番, 検査番号, 検査日付]=[%s, %s, %s]'
                                         % (app_id, app_name, fabric_name, inspection_num, inspection_date))
                            sys.exit()
                    else:
