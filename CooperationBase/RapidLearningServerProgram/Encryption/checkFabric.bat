...
echo MsgBox "端判定結果ファイル出力先選択画面に遷移します。" ^& vbCr ^& "端判定結果ファイル出力先・ファイル名を指定してください。", vbOKOnly + vbInformation, "情報" > %TEMP%\chosefoldermsgbox.vbs & %TEMP%\chosefoldermsgbox.vbs
...
if %ERRORLEVEL% == 0 (
  echo MsgBox "端判定が完了しました。", vbOKOnly + vbInformation, "情報" > %TEMP%\msgbox.vbs & %TEMP%\msgbox.vbs
) else (
  echo MsgBox "端判定実行時にエラーが発生しました。" ^& vbCr ^& "エラーログを確認してください。" ^& vbCr ^& "" , vbOKOnly + vbCritical, "エラー" > %TEMP%\error_msgbox.vbs & %TEMP%\error_msgbox.vbs
)
