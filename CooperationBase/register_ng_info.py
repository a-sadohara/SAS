...
def select_processing_target(conn, cur, fabric_name, inspection_num, imaging_starttime, unit_num):
    ...
    sql = 'select ii.inspection_direction ' \
          ...
          'where imageprocessing_starttime IS NOT NULL and ng_endtime IS NULL and status <> 0 ' \
          'and fi.fabric_name = \'%s\' and fi.inspection_num = %s and fi.imaging_starttime = \'%s\' '\
...
def main(product_name, fabric_name, inspection_num, imaging_starttime):
    ...
    try:
        ...
        while True:
            ...
            # 検査情報テーブルから検査情報を取得する
            result, fabric_info, error, conn, cur, func_name = select_processing_target(conn, cur, fabric_name, inspection_num, imaging_starttime, unit_num)
            ...
            if len(fabric_info) != 0:
                ...
                result, regimark_info, error, conn, cur, func_name = \
                ...
                while True:
                    if len(regimark_info) == 0:
                        ...
                        result, ng_image_info, error, conn, cur, func_name = \
                        ...
                        result, imageprocess_endtime, error, conn, cur, func_name = \
                            select_imageprocess_time(conn, cur, fabric_name, inspection_num,
                        ...
                        # NG画像が1枚以上で後続処理も完了している場合、全画像の結果一律で登録する。(レジマークがなく計算できないため)
                        if len(ng_image_info) != 0 and imageprocess_endtime is not None:
                            ...
                            result, error, conn, cur, func_name = \
                                update_fabric_info(conn, cur, fabric_name, inspection_num, fabric_ng_start_status,
                                                   fabric_ng_start_column, imaging_starttime, unit_num)
                            ...
                            result, error, conn, cur, func_name = \
                                update_rapid_analysis_all(conn, cur, fabric_name, inspection_num,
                            ...
                            logger.debug('[%s:%s] 反物情報テーブルの更新を開始します。' % (app_id, app_name))
                            result, error, conn, cur, func_name = \
                                update_fabric_info(conn, cur, fabric_name, inspection_num, fabric_ng_end_status,
                                                   fabric_ng_end_column, imaging_starttime, unit_num)
                            if result:
                        ...
                        # NG画像が0枚で後続処理も完了している場合、NG算出対象はないので、検査完了と判断。
                            ...
                            logger.debug('[%s:%s] 後処理終了時刻が存在しません。' % (app_id, app_name))
                        ...
                        logger.debug('[%s:%s] 後処理終了時刻取得を開始します。' % (app_id, app_name))
                        result, imageprocess_endtime, errro, conn, cur, func_name = \
                            select_imageprocess_time(conn, cur, fabric_name, inspection_num,
                                                     imaging_starttime, unit_num)
                        ...
                        if len(ng_image_info) != 0:
                            ...
                            result, mst_data, error, conn, cur, func_name = \
                            ...
                            for j in range(len(ng_image_info)):
                                ...
                                result, line_info, last_flag, error, func_name = \
                                ...
                                if last_flag == 0:
                                    ...
                                    if len(line_info) == 1:
                                        ...
                                        line_num = line_info[1][0]

                                logger.debug('[%s:%s] %s行目のレジマーク間長さ比率算出を開始します。レジマーク情報 [%s]' % (
                                    app_id, app_name, line_num, line_info))
                                result, regimark_length_ratio, conf_regimark_between_length_pix, \
                                error, func_name = \
                                ...
                                if result:
                                    logger.debug('[%s:%s] レジマーク間長さ比率算出が終了しました。' % (app_id, app_name))
                                    logger.debug('[%s:%s] レジマーク間長さ比率 [%s]' % (
                                ...
                                result, length_on_master, width_on_master, ng_face, error, func_name = \
                                    register_ng_info_util.specific_ng_point(line_info, ng_image_info[j], nonoverlap_image_width_pix,
                                ...
                                result, judge_result, length_on_master, width_on_master, error, func_name = \
                                    register_ng_info_util.specific_ng_line_colum(
                                ...
                                if result == True and judge_result == None:
                                    ...
                                    result, judge_result, length_on_master, width_on_master, error, func_name = \
                                        register_ng_info_util.specific_ng_line_colum_border(
                                        line_info, length_on_master, width_on_master, mst_data,
                                        conf_regimark_between_length_pix, inspection_direction, last_flag, logger)
                                    if result:
                                        logger.debug(
                        ...
                        # 反物情報ステータスを取得する。
                        result, fabric_marikingendtime, error, conn, cur, func_name = \
                        ...
                        # 端判定・マスキング判定結果を取得する
                        result, processed_count, error, conn, cur, func_name = \
